...
#设置几个目录
setwd("D:\\project0311") # csv文件路径
path.plot = "D:\\project0311\\plot" # 绘图路径
path.rasters = "D:\\project0311\\data" # data路径

#######################################################################
##第一部分：建立随机森林模型
#输入中华按蚊发生点的数据

sample.presence<-read.csv("sample_presence.csv",header=T)
...
#输入中华按蚊不发生点的数据
sample.absence<-read.csv("sample_absence.csv",header=T)
...
#利用随机森林的自带参数（sampsize）对数据进行平衡。平衡比例为1：1，即中华按蚊发生点数据样本量和不发生样本量相等
for(i in 1:100){
  set.seed(i*10)
  train$dataType = factor(train$dataType)
  test$dataType = factor(test$dataType)
  rf<-randomForest(dataType~.,train,ntree=100,xtest=test[,1:5],ytest=test$dataType,
                   sampsize=c(nrow(presence.train),nrow(presence.train)),strata=train$dataType,keep.forest=T)
...
error.bar(barx,apply(VarImportance,2,mean), 1.96*apply(VarImportance,2,sd)/sqrt(100))   #apply(,2/1)：2表示列，1表示行
dev.off()
...
pred = raster::predict(predictors, Maxrf, filename="Probability of presence.tif", type="prob", 
